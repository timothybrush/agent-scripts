...
- Maintain at least 10 active root-owned item threads while ten qualified independent items exist. Use one isolated Codex worktree thread per issue or PR, title it `OC <ref>: <current status>`, and prohibit worker delegation. The root orchestrator alone creates, steers, archives, and refills these lanes.
