...
- Added an explicit OpenClaw mode with root-session-only discovery and triage, 20 isolated `OC`-named execution worktrees for already-qualified tasks, adoption of existing lanes, root-only owner questions, Discord-sourced external-contributor discovery, live GitHub permission filtering, Vision-based autonomy, repo-native serialized landing, and OpenClaw-specific proof and changelog rules.
- Made activation create or update a five-minute root-session monitoring heartbeat, raised generic and OpenClaw execution concurrency to 20 qualified lanes, and made answered owner decisions advance immediately to the next prepared question while autonomous work continues.
