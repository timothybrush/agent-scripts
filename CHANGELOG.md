...
- Added an explicit OpenClaw mode with root-session-only discovery and triage, ten isolated `OC`-named execution worktrees for already-qualified tasks, adoption of existing lanes, root-only owner questions, Discord-sourced external-contributor discovery, live GitHub permission filtering, Vision-based autonomy, repo-native serialized landing, and OpenClaw-specific proof and changelog rules.
